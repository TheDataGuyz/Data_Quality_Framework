        ...
        SET @result_script = 'INSERT INTO PRODUCT.SECURITY.DQ_RESULT (
                              ...
                              SELECT 
									...
									,CASE
									 ...
									 ,COALESCE(ROUND((CAST((SUM(CASE WHEN ' + @RULE_VALIDATION_SCRIPT + ' THEN 1 ELSE 0 END)) AS FLOAT) / COUNT(*)) * 100, 2), 0) AS PCT_OF_FAILED
									 ,COALESCE(ROUND((CAST((SUM(CASE WHEN ' + @RULE_VALIDATION_SCRIPT + ' THEN 0 ELSE 1 END)) AS FLOAT) / COUNT(*)) * 100, 2), 0) AS PCT_OF_PASSED
									 ,CURRENT_TIMESTAMP
                              FROM ' + @DATABASE_NAME + '.' + @SCHEMA_NAME + '.' + @TABLE_NAME + ';';
