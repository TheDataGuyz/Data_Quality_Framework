CREATE TABLE PRODUCT.SECURITY.DQ_RULE (
	...
	PII_FLAG VARCHAR(10),
    RULE_TYPE VARCHAR(50), -- it is used as an identifier in proc to how the execution works
...
--DROP TABLE PRODUCT.SECURITY.DQ_RULE
--DROP TABLE PRODUCT.[Security].[DQ_RESULT]
--DROP TABLE PRODUCT.[Security].[DQ_FAILED_RECORDS]
--DROP TABLE PRODUCT.[Security].[DQ_RUN_STATUS]
--DROP TABLE PRODUCT.[Security].[DQ_ERROR_HANDLE_LOG]

-- a trigger to update the UPDATED_TIMESTAMP column on UPDATE
...
BEGIN
    ...
    IF UPDATE(SUBJECT_AREA) OR UPDATE(DATABASE_NAME) OR UPDATE(SCHEMA_NAME) 
		OR UPDATE(TABLE_NAME) OR UPDATE(COLUMN_NAME) OR UPDATE(PII_FLAG)
        OR UPDATE(PRIMARY_KEY_COLUMN) OR UPDATE(RULE_CATEGORY) 
		OR UPDATE(RULE_THRESHOLD) OR UPDATE(RULE_SEVERITY)
        OR UPDATE(RULE_DESCRIPTION) OR UPDATE(RULE_VALIDATION_SCRIPT) 
		OR UPDATE(DQ_ACTIVE_FG) OR UPDATE(RULE_TYPE)
    BEGIN
